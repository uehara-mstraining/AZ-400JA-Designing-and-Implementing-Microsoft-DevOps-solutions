...
1.  **Enabling Continuous Integration with Azure Pipelines**プロジェクトで Azure DevOps 組織を表示している Web ブラウザーで、垂直ナビゲーション ペインで、「**Pipelines**」 セクションを選択し、「**Pipelines**」 ビューが表示されていることを確認します。
...
1.  **PartsUnlimitedE2E** ペインの 「**Tasks**」 タブで、「**Triggers**」 タブをクリックし、「**Enable continuous integration**」 チェックボックスをオフにして、「**Save & queue**」の右側の下矢印をクリックし、「**Save**」 をクリックします。
...
1. **Azure Repos Git** オプションを選択し、 **PartsUnlimited** リポジトリと**master** ブランチ エントリが選択されていることを確認し、「**Continue**」 をクリックします。
1.  「**Select a template**」 ペインの 「**Search**」 テキスト ボックスに「**ASP.NET**」と入力し、結果のリストで **ASP.NET** テプレートを選択し、「**Apply**」 をクリックしてこのテンプレートをビルド定義に適用します。
...
1.  「**Retention**」 タブを選択します。「**go to the project settings to configure**」 リンクを右クリックして、「**Open link in new tab**」 を選択します。
1.  新しく開いたタブに切り替え、プロジェクト設定の 「**Pipelines**」 セクションの 「**Settings**」 ペインを確認して、タブを閉じます。
...
1.  「**Save & queue**」 タブ ヘッダーを選択し、ドロップダウン メニューで 「**Save & queue**」 エントリを選択して、「**Save & queue**」を選択します。
...
1.  ビルドが正常に完了したら、ジョブの詳細ペインで、左向きの矢印をクリックして**Summary**ビューに戻ります。

    > **注**: **Summary**ビューには、コミット、テスト、アーティファクトに関する詳細など、ビルドに関する概要の詳細が表示されます。
...
1.  「**Pipelines**」 ペインで、変更によってトリガーされた新しいビルドエントリが含まれていることを確認します (Last run に表示される番号の末尾に **.2** が含まれていることに注意してください)。
